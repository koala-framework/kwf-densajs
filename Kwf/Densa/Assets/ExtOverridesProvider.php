...
class Kwf_Densa_Assets_ExtOverridesProvider extends Kwf_Assets_Provider_Abstract
{
    ...
    private static function _getOverrides()
    {
        ...
        $ret = array();
        $it = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(VENDOR_PATH.'/koala-framework/densajs/src/overrides'), RecursiveIteratorIterator::LEAVES_ONLY);
        foreach ($it as $i) {
            if (substr($i->getPathname(), -3) != '.js') continue;
            $depName = 'Densa.'.str_replace('/', '.', substr($i->getPathname(), strlen(VENDOR_PATH.'/koala-framework/densajs/src/'), -3));
